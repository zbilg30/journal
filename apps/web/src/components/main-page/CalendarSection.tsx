...
import type { CalendarDay, CalendarWeek, TradeDayData } from './types'
...
function WeekSummary({ week, formatCurrency, className }: { week: CalendarWeek; formatCurrency: (value: number) => string; className?: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1 text-left lg:text-right">
        <p className="text-lg font-semibold tracking-tight sm:text-xl">{formatCurrency(week.total)}</p>
...
function MobileTradeDetail({ activeTrade, setupNameMap }: { activeTrade?: TradeDayData; setupNameMap: Record<string, string> }) {
  if (!activeTrade) {
    return <p className="text-sm text-muted-foreground">No trades logged for this day.</p>
  }
    ...
    <div className="space-y-4">
      <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <dd className="text-sm font-semibold text-foreground">{activeTrade.setupId ? setupNameMap[activeTrade.setupId] ?? activeTrade.setupId : '—'}</dd>
        </div>
        <div className="flex items-center justify-between gap-3">
    ...
    <section className="rounded-[32px] bg-white/80 p-6 shadow-xl shadow-purple-400/25 ring-1 ring-black/5 backdrop-blur">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
          <span className="rounded-full border border-muted/60 bg-muted/40 px-3 py-1">{monthlyActiveDays} days</span>
          <span className="rounded-full border border-muted/60 bg-muted/40 px-3 py-1">{monthlyTradeCount} trades</span>
        </div>
      </div>
        ...
        {isMobileView ? (
          <div className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch sm:gap-4">
              ...
              {activeWeek ? <WeekSummary className="w-full" formatCurrency={formatCurrency} week={activeWeek} /> : null}
            </div>

            <div className="rounded-3xl border border-muted/40 bg-white/90 p-4 text-sm shadow-sm shadow-black/[0.02]">
              <MobileTradeDetail activeTrade={activeTrade} setupNameMap={setupNameMap} />
            </div>
          </div>
        ) : (
          <>
              ...
              {weeks.map((week) => (
                <div key={week.id} className="grid gap-3 lg:grid-cols-8">
                  ...
                  <div className="lg:col-span-1">
                    <WeekSummary formatCurrency={formatCurrency} week={week} />
                  </div>
                </div>
              ))}
            </div>
